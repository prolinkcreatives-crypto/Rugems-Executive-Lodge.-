...
import { checkInDateIssue, checkInIssueMessage } from "@/lib/lodge-time";
...
export const listSanctuaries = createServerFn({ method: "GET" }).handler(async () => {
  const { data, error } = await serverPublic()
    .from("sanctuaries")
    .select("slug,name,tagline,location,description,price_per_night,size_sqm,amenities,hero_image,gallery,sort_order,capacity")
    .eq("active", true)
    .order("sort_order");
...
export const getSanctuary = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    const { data: row, error } = await serverPublic()
      .from("sanctuaries")
      .select("slug,name,tagline,location,description,price_per_night,size_sqm,amenities,hero_image,gallery,capacity")
      .eq("slug", data.slug)
      .eq("active", true)
      .maybeSingle();
    if (error) throw new Error(error.message);
...
const bookingSchema = z
  ...
  .refine(
    (v) => checkInDateIssue(v.checkIn) === null,
    (v) => ({
      message: checkInIssueMessage(checkInDateIssue(v.checkIn)) ?? "Invalid check-in date.",
      path: ["checkIn"],
    }),
  );
...
export const createBooking = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // Look up sanctuary to compute price / capture name / enforce capacity
    ...
    if (data.guests > sanctuary.capacity) {
      throw new Error(`${sanctuary.name} accommodates up to ${sanctuary.capacity} guests.`);
    }
    ...
    try {
      const { sendOquMailEmail } = await import("@/lib/oqumail.server");
