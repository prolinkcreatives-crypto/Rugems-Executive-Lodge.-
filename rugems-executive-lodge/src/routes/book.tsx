...
import {
  checkInDateIssue,
  checkInIssueMessage,
  earliestSelectableCheckInISO,
  earliestSelectableCheckOutISO,
  CHECKOUT_BEFORE_CHECKIN_MESSAGE,
  type CheckInIssue,
} from "@/lib/lodge-time";
import { useSiteImage, listSiteImages, type SiteImageSlotKey } from "@/lib/site-images";
...
export const Route = createFileRoute("/book")({
  ...
  loader: ({ context }) =>
    Promise.all([
      context.queryClient.ensureQueryData(sanctuariesQuery),
...
function reviewBlockingReason(form: FormState): string | null {
  ...
  const checkInIssue = checkInDateIssue(form.checkIn);
  if (checkInIssue) return checkInIssueMessage(checkInIssue);
  return null;
}
...
function BookPage() {
  ...
  // Re-checked periodically, not just when the form changes, so the
  // same-day cutoff (and "today" itself) stay correct if a guest leaves
  // the tab open across the cutoff moment or midnight.
  ...
  const checkInIssue: CheckInIssue = useMemo(() => checkInDateIssue(form.checkIn, now), [form.checkIn, now]);
  const checkInErrorText = checkInIssueMessage(checkInIssue) ?? undefined;
  const checkOutBeforeOrEqual = Boolean(form.checkIn && form.checkOut && form.checkOut <= form.checkIn);
  ...
  const canAdvance = () => {
    ...
    if (step === 1) return nights > 0 && form.guests > 0 && checkInIssue === null && (!selected || form.guests <= selected.capacity);
                    ...
                    <div className="space-y-10">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <FloatingInput
                          ...
                          type="date"
                          value={form.checkIn}
                          min={earliestSelectableCheckInISO(now)}
                        ...
                        <FloatingInput
                          ...
                          type="date"
                          value={form.checkOut}
                          min={earliestSelectableCheckOutISO(form.checkIn, now)}
                          error={checkOutBeforeOrEqual ? CHECKOUT_BEFORE_CHECKIN_MESSAGE : undefined}
                          onChange={(v) => {
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-3">
                          {GUEST_OPTIONS.filter((g) => !selected || g.count <= selected.capacity).map((g) => (
                            <GuestChip
                              key={g.count}
                              option={g}
                        ...
                        {selected && (
                          <p className="text-label-caps text-on-surface-variant mt-3 opacity-70">
                            {selected.name} accommodates up to {selected.capacity} guests.
                          </p>
                        )}
                      </div>
...
const LocationCard = memo(function LocationCard({
  ...
  const image = useSiteImage(`location-${loc.slug}` as SiteImageSlotKey, loc.image);
  return (
    <motion.button
      type="button"
...
function FloatingInput({
  ...
  const complete = value.length > 0 && !focused && confirmedLabel && !error;
  return (
    <div className="relative pt-6">
      <motion.label
        animate={{
          ...
          color: error ? "var(--destructive)" : focused ? "var(--primary)" : complete ? "var(--gold)" : "var(--on-surface-variant)",
        }}
        transition={{ duration: 0.3, ease: cinematic }}
      ...
      <input
        ...
        className={
          ...
          (error ? "border-destructive" : complete ? "border-gold" : focused ? "border-primary" : "border-outline-variant")
        }
      />
      {error && <p className="mt-2 text-label-caps text-destructive">{error}</p>}
    </div>
  );
}
